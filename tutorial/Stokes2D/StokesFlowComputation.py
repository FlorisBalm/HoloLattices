...
from tutorial.Stokes2D.util.ThermodynamicsStokes import FullThermodynamics
...
def run_2d(background, setup_file, output):
    configuration = {
    ...
    mixed_basis = True
    if mixed_basis:
        basis_0 = 0.5 * np.array([1, 1, 1, 1])
        basis_1 = 0.5 * np.array([1, -1, 1, -1])
        basis_2 = 0.5 * np.array([1, 1, -1, -1])
        basis_3 = 0.5 * np.array([1, -1, -1, 1])
    ...
    mu0 = mu = np.average(Thermodynamics["Mu"])
    ...
    rhoH = np.average(result_basis2["rhoH"]) / mu ** 2
    ...
    data_ret = {
        ...
        "SigmaQ011": SigmaQ0[0, 0], "SigmaQ012": SigmaQ0[0, 1], "SigmaQ021": SigmaQ0[1, 0], "SigmaQ022": SigmaQ0[1, 1],
        "Mu": Mu, "Ttt": Ttt, "Txx": Txx, "Tyy": Tyy, "Txy": Txy, "rho": rho, "rhoH": rhoH, "T": T,
        "S": S,
        "Omega": np.average(Thermodynamics["Omega"]),
        "EInternal": np.average(Thermodynamics["EInternal"])}
    ...
    for idx, background in enumerate(parsed.backgrounds):
        ...
        with tempfile.NamedTemporaryFile(mode="w") as temp_file:
            ...
            with h5py.File(background, "r") as infile_par:
                ...
                T = (12 - mu ** 2 - B ** 2) / (16 * np.pi * mu)
                A0 = parameters[2]
                lx = parameters[4]
                nperiods = parameters[6]
                P = (2 * np.pi * nperiods) / (lx * mu)
                c1 = parameters[11]

            temp_file.flush()

            data = run_2d(setup_file=temp_file.name, output=None, background=background)
            data["T"] = T
            data["A0"] = A0
            data["B"] = B
            data["P"] = P
            data["muTB"] = mu
            data["c1"] = c1
            dataframe = dataframe.append(pandas.DataFrame(data))
    import os
